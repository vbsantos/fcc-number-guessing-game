...
declare PSQL="psql --username=freecodecamp --dbname=number_guessing_game -t --no-align -c"

# Generate random number between 1 to 1000
declare secret_number=$((RANDOM % 1000 + 1))

# Prompt user for username
read -p "Enter your username: " username

# Check if user already exists in the database
declare user_exists="$($PSQL "SELECT * FROM users WHERE username = '${username}';")"

if [ -n "${user_exists}" ]; then
    # Extract data for existing user
    IFS='|' read -r -a arr <<< "$user_exists"
    declare number_of_games="${arr[2]}"
    declare best_game="${arr[3]}"
    ...
    declare create_user="$($PSQL "INSERT INTO users (username,number_of_games,best_game) VALUES ('${username}',0,1000);")"
fi
